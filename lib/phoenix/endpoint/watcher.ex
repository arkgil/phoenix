defmodule Phoenix.Endpoint.Watcher do
  ...
  def watch(cmd, args, opts) do
    merged_opts = Keyword.merge([into: IO.stream(:stdio, :line), stderr_to_stdout: true], opts)
  ...
  defp validate("node", [script | _], merged_opts) do
    ...
    cond do
      ...
      not File.exists?(script_path) ->
        Logger.error(
          "Could not start node watcher because script #{inspect(script_path)} does not " <>
