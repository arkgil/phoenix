defmodule Phoenix.Controller.Pipeline do
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      def call(conn, action) when is_atom(action) do
        ...
        :telemetry.execute([:phoenix, :controller, :call, :stop], %{total: diff}, %{
          conn: conn,
          controller: controller,
          action: action
        })
  ...
  defmacro __before_compile__(env) do
    ...
    {conn, body} =
      Plug.Builder.compile(env, plugs, log_on_halt: :debug, init_mode: Phoenix.plug_init_mode())
  ...
  def __catch__(
        ...
        controller,
        action,
        [{controller, action, [%Plug.Conn{} = conn | _], _loc} | _] = stack
      ) do
